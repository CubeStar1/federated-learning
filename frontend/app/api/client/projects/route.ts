...
export async function GET() {
  try {
    ...
    const summaries: ProjectSummary[] = projects.map((project) => {
      ...
      const coordinatorCount = projectNodes.filter((node) => node.role === "coordinator").length;
      const participantCount = projectNodes.filter((node) => node.role === "participant").length;
      const nodeCount = projectNodes.length;
      const activeRunCount = projectRuns.filter((run) => run.status === "running").length;
